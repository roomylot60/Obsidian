...
- 이러한 BLEU score 평가 방식은 [2](https://arxiv.org/abs/1409.0473) 와 [5](https://arxiv.org/abs/1406.1078)의 내용과 일치하고 [29](H. Schwenk. University le mans. http://www-lium.univ-lemans.fr/˜schwenk/cslm_ joint_paper/, 2014. [Online; accessed 03-September-2014].)에서의 33.3점을 재확인했다.
- 그러나 우리가 이러한 수단으로 best WMT'14 system([링크](statmt.org\matrix)에서 다운로드 가능한)을 평가했을 때, 우리는 링크에서 언급한 35.8점을 넘는 37.0점을 얻었다.

- 이러한 결과는 다음 테이블들에서 확인 가능하다.
![](Attatched/Pasted%20image%2020240330212619.png)

![](Attatched/Pasted%20image%2020240330212644.png)
- 우리의 최고 결과들은 랜덤 초기화와 minibatch의 순서를 랜덤하게 배치한 차이를 갖는 LSTM들의 종합을 통해 얻었다.
- 디코딩 된 LSTM들의 종합으로 얻은 번역이 best WMT'14 system보다 뛰어나지 않지만, 순수 신경망 번역 시스템이 사전 외 단어를 다루는 것이 불가능함에도 불구하고 거대규모 MT 작업에서 구문 기반 SMT 기반에 비해  규모 대비 이득 측면에서 뛰어난 것은 처음이었다.
- LSTM이 기본 시스템의 1000-best 리스트를 재평가했을 때, WMT'14의 결과에서 0.5 BELU 점이내에 위치한다.
### 3.7 Performance on long sentences

- 다음 그래프에서 수치적으로 볼 수 있듯, LSTM이 긴 문장들에 대해서 잘 작동하는 점은 놀라웠다.
![](Attatched/Pasted%20image%2020240330212512.png)
- 다음 표는 긴 문장들과 그 번역들에 대한 몇몇 예시를 보여준다.
![](Attatched/Pasted%20image%2020240330212323.png)
### 3.8 Model Analysis

- 제시한 모델의 장점 중 하나는 단어 시퀀스를 고정된 차원의 벡터로 전환하는 능력이다.
- 다음 그래프는 학습한 표현들에 대해 시각화하여 보여준다.
![](Attatched/Pasted%20image%2020240330212554.png)
- 그래프는 해당 표현 방식들이 능동태와 수동태의 대체에서는 상당히 둔감한 반면, 단어의 순서에 민감하다는 것을  선명히 보여준다.
- 2차원 투사는 PCA를 통해 얻었다.
## 4. Related Work

- 신경망을 MT에 적용하는 것은 상당히 거대한 작업이었다.
- 지금까지 RNNLM이나 FFNNLM을 MT 작업에 적용하는 가장 간단하고 효율적인 방식은 확실하게 번역의 질을 높이는 강력한 MT 기반 모델로 n-best 리스트를 재평가하는 것이었다.
- 최근에는 연구자들이 source language에 대한 정보를 NNLM에 포함시키는 방식을 모색하기 시작했다.
- [1](https://aclanthology.org/D13-1106/)에서 Auli 등의 연구가 이러한 작업을 포함하는 일례로, NNLM을 입력 시퀀스의 주 모델로 통합시켜 재평가 성능을 증가시켰다.
- [8](https://aclanthology.org/P14-1129/)에서의 Devlin 등의 연구에서 비슷한 접근을 하였으나, NNLM을 MT 시스템의 디코더로 포함시키지 못하였고, 디코드의 정렬 정보를 입력 시퀀스에서 가장 유용한 단어들을 사용하여 제공하는 NNLM에 사용하였다. 
- 그들의 접근법은 상당히 성공적이었고 기본 모델 전반에서 큰 발전을 달성했다.

- 우리의 작업은 [KAlchbrenner와 Blunsom의 연구](https://aclanthology.org/D13-1176/)와 밀접한 관련이 있다. 그들의 매핑하는 방식이 CNN을 사용하여 단어들의 순서를 잃어버리긴 하지만, 처음으로 입력 시퀀스를 벡터로 매핑하고 문장으로 되돌리는 방법이었다.
- 이러한 작업과 비슷하게 [Cho 들의 연구]()에서 신경망을 SMT 시스템과 결합하는 데 가장 초점을 맞추기는 했으나, LSTM과 비슷한 RNN 구조를 문장을 벡터로 매핑하고 되돌리는 데 사용했다.
- Bahdanau 들의 연구에서도 Cho들의 연구에서의 긴 문장에 대한 낮은 성능을 극복하고자 Attention 메커니즘을 적용한 신경망으로 직접 번역을 시도했고 더 나은 결과를 얻었다.
- [Pouget-abadie 들]()도 Cho들의 메모리 문제를 해결하기 위해 구문 기반 접근법과 비슷하게 source sentence의 조각들을 번역하여 매끄러운 문장을 생산하는 방식을 시도했다.
- 그들이 reversed source sentence를 그들의 신경망에 적용하여 학습했다면 우리의 연구와 비슷한 결과를 달성했을 것이라고 생각한다.

- End-to-end 학습은 Hermann들의 연구의 쟁점이다. 그들의 모델은 입,출력을 FFNN을 사용하여 표현하고 비슷한 공간상의 위치를 매핑한다.
- 그러나 그들의 접근법은 직접적으로 번역을 생성하지 못한다: 해당 방식에서 번역을 얻기 위해선, 사전에 연산한 문장 데이터베이스에서 가장 근접한 벡터를 찾거나 문장을 재평가해야 한다. 
---

## 5. Conclusion

- 이 연구에서 우리는 제한된 사전을 갖고, 거대 규모 MT 작업에서 사전이 무제한인 표준 SMT 기반 시스템보다 뛰어난 성능을 가지고 문제 구조에 대해 거의 추정을 하지 않는 거대 심층 LSTM을 선보였다.
- MT에서의 Simple LSTM 기반 접근법의 성공은 충분한 학습 데이터가 제동되었을 때 다른 시퀀스 학습들에 대해서도 잘 작동한다는 것을 보여준다.

- 우리는 source sentence의 단어들의 순서를 뒤집음으로써 얻을 수 있는 성능 향상이 놀라웠다.
- 우리는 학습 문제를 더욱 간단하게 만들기 때문에 많은 수의 단기 의존성을 갖는 문제를 인코딩하는 것을 찾는 것이 중요하다고 결론지었다.
- 특히, 우리가 표준 RNN을 non-reversed 번역 문데에 적용시켜 학습하는 것은 못했으나, source sentence가 반전되었을 때, 표준 RNN이 더 학습하기 간단할 것이다(실험을 통해 확인하지는 못하였으나).

- LSTM이 긴 문장에서 올바르게 번역하는 능력 또한 놀라웠다.
- 처음에는 LSTM이 다른 연구에서 우리와 비슷한 모델에서 낮은 성능을 보였고, 제한된 메모리 때문에 긴 문장에서 실패할 것이라고 확신했다.
- 그러나 아직 역전된 데이터 셋에서 학습된 LSTM은 장문 번역에서 약간의 어려움이 있다.

- 가장 중요하게도, 우리는 간단하고 직접적인, 상대적으로 최적화되지 않은 접근법이 SMT 시스템을 능가할 수 있을 뿐 아니라, 해당 작업이 더 나은 번역 정확성을 이끌 것임을 보였다.
- 이러한 결과들은 우리의 접근법이 다른 어려운 시퀀스에서 시퀀스로의 문제들에서도 잘 작동할 것임을 보여준다.
---
