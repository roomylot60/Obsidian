...
- 따라서 신경망은 전통적인 통계적 모델과 관련이 있지만, 복잡한 계산을 학습합니다. 또한, 큰 DNNs는 라벨이 지정된 훈련 세트가 네트워크의 매개변수를 명시하기에 충분한 정보를 가질 때 지도 학습 역전파로 훈련될 수 있습니다. 따라서 큰 DNN의 매개변수 설정이 좋은 결과를 달성하는 경우(예를 들어, 인간이 빠르게 문제를 해결할 수 있기 때문에) 지도 학습 역전파는 이러한 매개변수를 찾아 문제를 해결합니다.
