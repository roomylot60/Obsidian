...
- 하나의 LSTM을 입력 시퀀스를 한 번에 한 시점씩 읽어내어 큰 고정 차원의 벡터 표현을 얻은 다음, 그 벡터에서 출력 시퀀스를 추출하기 위해 다른 LSTM을 사용하는 것입니다(그림 1). 두 번째 LSTM은 기본적으로 입력 시퀀스에 조건이 부여된 반복적인 신경망 언어 모델[28, 23, 30]입니다. LSTM은 긴 시간 의존성을 가진 데이터에서 성공적으로 학습할 수 있는 능력 덕분에 입력과 해당하는 출력 사이에 상당한 시간 지연이 있기 때문에 이 응용에 자연스럽게 적합한 선택지입니다(그림 1).